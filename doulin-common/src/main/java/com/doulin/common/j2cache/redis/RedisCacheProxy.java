...
import com.doulin.common.j2cache.redis.client.RedisClient;
import com.doulin.common.j2cache.redis.support.RedisClientFactoryAdapter;
...
public class RedisCacheProxy implements Closeable {
    ...
    public Long incr(byte[] fieldKey, int expireInSec) {
    	RedisClient redisClient = null;
    	try {
    		redisClient = getResource();
    		redisClient.expire(fieldKey, expireInSec);
    		return redisClient.incr(fieldKey);
    	} finally {
    		returnResource(redisClient);
    	}
    }
